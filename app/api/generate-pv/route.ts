...
export async function POST(req: NextRequest) {
  ...
  // Split students into groups of 20 (recommended 15 per page but max 20)
  const groups: typeof students[] = []
  for (let i = 0; i < students.length; i += 20) {
    groups.push(students.slice(i, i + 20))
  }

  function makeSectionChildren(group: typeof students, isFirst: boolean) {
    const rows = []

    // Fill to 20 rows
    const displayRows = [...group]
    while (displayRows.length < 20) displayRows.push(null)
    ...
    const headerRow = new TableRow({
      ...
      children: [
        new TableCell({ borders, width: { size: 700, type: WidthType.DXA }, margins: cellMargins,
          ...
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'Nr. crt', bold: true, size: 16 })] })] }),
        new TableCell({ borders, width: { size: 3500, type: WidthType.DXA }, margins: cellMargins,
          shading: { fill: 'D0D0D0', type: ShadingType.CLEAR },
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'NUMELE SI PRENUMELE', bold: true, size: 16 })] })] }),
        ...
        new TableCell({ borders, width: { size: 1200, type: WidthType.DXA }, margins: cellMargins,
          shading: { fill: 'D0D0D0', type: ShadingType.CLEAR },
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'CLASA CAA', bold: true, size: 16 })] })] }),
        new TableCell({ borders, width: { size: 2000, type: WidthType.DXA }, margins: cellMargins,
          shading: { fill: 'D0D0D0', type: ShadingType.CLEAR },
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'STATUS Admis/Respins/Absent', bold: true, size: 16 })] })] }),
      ]
    })

    for (let idx = 0; idx < displayRows.length; idx++) {
      const s = displayRows[idx]
      const rowNum = group[0] === students[0] ? idx + 1 : idx + 1
      rows.push(new TableRow({
        height: { value: 400, rule: 'atLeast' },
        children: [
          new TableCell({ borders, width: { size: 700, type: WidthType.DXA }, margins: cellMargins,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: s ? String(s.order_in_session || idx + 1) : '', size: 16 })] })] }),
          new TableCell({ borders, width: { size: 3500, type: WidthType.DXA }, margins: cellMargins,
            children: [new Paragraph({ children: [new TextRun({ text: s?.full_name || '', size: 16 })] })] }),
          new TableCell({ borders, width: { size: 2200, type: WidthType.DXA }, margins: cellMargins,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: s?.cnp || '', size: 16 })] })] }),
          new TableCell({ borders, width: { size: 1200, type: WidthType.DXA }, margins: cellMargins,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: s?.class_caa || '', size: 16 })] })] }),
          new TableCell({ borders, width: { size: 2000, type: WidthType.DXA }, margins: cellMargins,
    ...
    const children: any[] = [
      ...
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
        children: [new TextRun({ text: 'MINISTERUL TRANSPORTURILOR ȘI INFRASTRUCTURII', bold: true, size: 20 })] }),
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
        children: [new TextRun({ text: 'AUTORITATEA NAVALĂ ROMÂNĂ', bold: true, size: 20 })] }),
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 },
        children: [new TextRun({ text: 'CĂPITĂNIA ZONALĂ CONSTANȚA', bold: true, size: 20 })] }),
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 },
        children: [new TextRun({ text: 'CĂPITĂNIA PORTULUI MANGALIA', bold: true, size: 20 })] }),

      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 },
        children: [new TextRun({ text: 'Anexa 12', size: 18 })] }),
      ...
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 200, after: 200 },
        children: [new TextRun({ text: 'Proces – Verbal  Examen Practic', bold: true, size: 28 })] }),
  ...
  const allChildren = makeSectionChildren(students, true)
