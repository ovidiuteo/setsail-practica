...
export default function BancomatView({ config, closed, expLabel }: { config: VoucherConfig; closed: boolean; expLabel: string }) {
  ...
  const sep = config.spendUrl.indexOf('?') === -1 ? '?' : '&'
  const spendHref = result
    ? `${config.spendUrl}${sep}voucher=${encodeURIComponent(result.token)}&email=${encodeURIComponent(result.email)}`
    : '#'
    ...
    return (
      ...
      >
        <div className="text-center mb-7">
          ...
          <h1 className="text-2xl sm:text-3xl font-black">{config.pageTitle}</h1>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-white/55 mt-3 max-w-xs mx-auto">{config.outOfOrderText}</p>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-7">
        ...
        <h1 className="text-2xl sm:text-3xl font-black">{config.pageTitle}</h1>
        <p className="text-sm text-white/60 mt-1.5 max-w-md mx-auto">{config.pageSubtitle}</p>
      </div>
      ...
      <div className="w-full max-w-md">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              >
                {phase === 'input' ? 'VOUCHER ATM' : (
                  <>CREDIT ATM <span className="font-bold" style={phase === 'cashed' ? { color: '#fb923c' } : undefined}>{phase === 'cashed' ? 0 : amount}€</span></>
                )}
              </span>
            ...
            {phase === 'cashed' && (
              <div className="text-center py-1">
                ...
                <p className="text-5xl font-black mt-1" style={{ color: '#f5c842' }}>
                  {amount}<span className="text-3xl align-top">€</span>
                </p>
        ...
        {phase === 'cashed' && result && (
          <div className="mt-5 ss-note-out">
            <Banknote token={result.token} email={result.email} amount={amount} desc={config.banknoteDesc} exp={expLabel} />
            ...
            <p className="text-center text-xs text-white/40 mt-3">
              Codul e valabil la înscriere doar cu adresa <span className="font-mono text-white/60">{result.email}</span>. Exp {expLabel}
            </p>
          </div>
        )}
      </div>
