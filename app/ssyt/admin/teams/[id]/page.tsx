...
import { ArrowLeft, Sailboat, Users, Edit, Crown } from 'lucide-react'
    ...
    .select(`
      *,
      ...
      memberships:ssyt_team_memberships(
        id, membership_type, status, start_date, is_editor,
            ...
            <table className="w-full text-sm">
              <thead style={{ background: '#f8f9fa', borderBottom: '1px solid #e5e7eb' }}>
                <tr>
                  ...
                  <th className="text-center px-5 py-3 text-xs font-medium uppercase tracking-wider text-gray-500">Rol portal</th>
                  <th className="text-left px-5 py-3 text-xs font-medium uppercase tracking-wider text-gray-500">Din</th>
                </tr>
              </thead>
              <tbody>
                {activeMembers.map((m: any) => {
                  const isSkipperRow = m.participant?.id === team.skipper_id
                  return (
                    <tr key={m.id} style={{ borderTop: '1px solid #e5e7eb' }}>
                      ...
                      <td className="px-5 py-3 text-center">
                        {isSkipperRow ? (
                          <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md" style={{ background: 'rgba(255,107,53,0.15)', color: '#FF6B35' }}>
                            <Crown size={11} /> Skipper
                          </span>
                        ) : (
                          <EditorToggle membershipId={m.id} initialValue={!!m.is_editor} participantName={m.participant?.full_name} />
                        )}
                      </td>
