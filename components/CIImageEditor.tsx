...
export default function CIImageEditor({ file, onConfirm, onCancel }: Props) {
  ...
  async function loadPDFPage(pdfFile: File, pageNum: number) {
    setImageLoaded(false)
    setPdfLoading(true)
    try {
      // Incarca PDF.js din CDN folosind obiectURL (metoda care functioneaza)
      ...
      // Folosim createObjectURL - exact ca in exemplul tau care functioneaza
      const url = URL.createObjectURL(pdfFile)
      const pdf = await pdfjsLib.getDocument(url).promise
      setPdfTotalPages(pdf.numPages)
      const page = await pdf.getPage(pageNum)
      // Detectam orientarea si forteaza portrait
      const vpNatural = page.getViewport({ scale: 1 })
      const rotate = vpNatural.width > vpNatural.height ? 90 : 0
      const viewport = page.getViewport({ scale: 2, rotation: rotate })
      // Randam pe canvas temporar (nu cel din UI)
  ...
  function startDrag(handle: Handle | 'move', clientX: number, clientY: number) {
  ...
  function doDrag(clientX: number, clientY: number) {
    ...
    const { px, py } = toPercent(clientX, clientY)
    const dx = px - startX, dy = py - startY, MIN = 5
    setCrop(() => {
      let { x, y, w, h } = sc
      if (handle === 'move') {
  ...
  const onMouseMove = useCallback((e: MouseEvent) => doDrag(e.clientX, e.clientY), [])
  ...
  function handleConfirm() {
    ...
    console.log(`CI OCR: ${canvas.width}x${canvas.height}px, ${Math.round(dataUrl.length/1024)}KB`)
    ...
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col" style={{maxHeight:'95vh'}}>
        ...
        {isPDF && pdfTotalPages > 1 && (
          <div className="flex items-center justify-center gap-3 px-4 py-2 bg-gray-50 border-b border-gray-100 shrink-0">
            <button onClick={()=>changePDFPage(pdfPage-1)} disabled={pdfPage<=1||pdfLoading}
            ...
            <button onClick={()=>changePDFPage(pdfPage+1)} disabled={pdfPage>=pdfTotalPages||pdfLoading}
              className="p-1.5 rounded-lg border border-gray-200 disabled:opacity-30 hover:bg-white">
        ...
        <div className="relative bg-gray-900 flex items-center justify-center overflow-hidden" style={{flex:1, minHeight:200, maxHeight:'58vh'}}>
          {!imageLoaded ? (
            ...
            <div className="relative inline-block select-none" style={{maxWidth:'100%', maxHeight:'58vh'}}>
              <canvas ref={canvasRef} style={{display:'block', maxWidth:'100%', maxHeight:'58vh', objectFit:'contain'}}/>
        ...
        <div className="px-3 py-2.5 border-t border-gray-100 shrink-0 bg-white">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={applyCrop}
              className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-medium transition-colors flex-1 ${isCropDefault?'border border-gray-200 text-gray-400 cursor-default':'bg-amber-500 text-white'}`}
