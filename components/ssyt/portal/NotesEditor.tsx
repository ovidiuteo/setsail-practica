...
export default function NotesEditor({
  ...
  async function save() {
    setBusy(true)
    const res = await fetch('/api/ssyt/portal/team-notes', {
      method: 'PUT',
    ...
    <div className="rounded-lg p-5" style={{ background: '#fff', border: '1px solid #e5e7eb' }}>
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm font-medium uppercase tracking-wider text-gray-500 inline-flex items-center gap-1.5">
          <FileText size={12} /> Note echipă
        </div>
        {canEdit && !editing && (
          <button onClick={() => setEditing(true)} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md hover:bg-gray-100 transition" style={{ color: '#FF6B35' }}>
      ...
      {editing ? (
        <div>
          <textarea
            ...
            rows={12}
            placeholder="Scrie aici note libere pentru echipă (markdown acceptat)..."
          ...
          <div className="flex items-center gap-2 justify-end mt-3">
            <button onClick={() => { setEditing(false); setContent(initialContent) }} className="px-3 py-1.5 text-sm text-gray-600 hover:text-gray-900">
        ...
        <div className="prose prose-sm max-w-none">
          {content.trim() ? (
            ...
            <p className="text-sm text-gray-400 italic">Nicio notă încă.{canEdit ? ' Adaugă prima notă cu click pe Editează.' : ''}</p>
          )}
        </div>
      )}
    </div>
